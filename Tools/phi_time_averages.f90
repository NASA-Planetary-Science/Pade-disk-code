...
! For Manger check:
integer, parameter :: lun_alpha1 = 9
...
real(8), allocatable, dimension(:, :) :: T_zz, T_pp, T_rr, T_rz, T_pr, T_pz, T_trace, &
                                         T_pr1 ! For Manger check

! For computing alpha:
real(8), allocatable, dimension(:) :: Sigma, uphi_ave, T_pr_int, T_pr1_integral, Omega_ave, &
                                      alpha
real(8) :: Lz, d_Omega_dr, nu_t, nu_t1, dz
...
integer :: n_yes, nbar, isel
character(2)  :: yorn
logical :: Manger_check
...
write(6, "(' Are you checking Manger expression (y/n)--->', $)")
read(5, "(a1)") yorn
if (yorn .eq. 'y') then
   Manger_check = .true.
   nbar = 11
...
if (Manger_check) then
   allocate(T_pr1(nr,nz))
   allocate(T_pr_int(nr))
end if
...
write(6, 17)
17 format(' enter 1: For analysis using phi-time averages',/, &
          '       2: To calculate alpha using time average of azimuthal means',/, &
          ' --->', $)
read(5, *) isel
...
! Manger's stress:
if (Manger_check) then
   T_pr1(:,:) = bar(rho_uphi_ur, :,:) - bar(rho_ur,  :,:)*bar(uphi_index,:,:)
end if
...
! Compute alpha:
T_pr_int  = 0.d0
if (Manger_check) T_pr1_integral = 0.d0
...
do iz = 1, nz - 1
   ...
   do ir = 1, nr
      ...
      if (Manger_check) then
         T_pr1_integral(ir) = T_pr1_integral(ir) + 0.5d0*(T_pr1(ir,iz) + T_pr1(ir,iz+1))*dz
      end if
   end do
end do
...
do ir = 2, nr - 1
   ...
   if (Manger_check) then
      nu_t1 = - T_pr1_integral(ir) / Sigma(ir) / rgrid(ir) / d_Omega_dr   
      write(lun_alpha1,"(2(1x, e12.5))") rgrid(ir), nu_t1/ (cs*H)
   end if
end do
close(lun_alpha)
close(lun_Tpr_int)
if (Manger_check) close(lun_alpha1)
