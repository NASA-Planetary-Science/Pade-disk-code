...
real(8) :: numer, denom, Kmodel, Tpz_model, C_r_phi, product
...
! This is also a parameter of the simulation:
q = -1.d0
...
! Header of output tecplot files:
...
open (unit = lun_model, file = 'Tphiz_model.tec', form = 'formatted', status = 'unknown')
write (lun_model, 14)
14  format ('TITLE = "Kmodel"')
write (lun_model, 15)
15  format('VARIABLES="r","z","Tpz_model", "C_r_phi", "product", ddz_upt')
write (lun_model, 16) nr-1, nz-1
...
do iz = 1, nz - 1
   do ir = 1, nr - 1
      ...
      numer = dataM(ir, iz, iTpz)
      ratio = dataM(ir, iz, iTzz) / dataM(ir, iz, irhob)
      denom = -dataM(ir, iz, irhob) * sqrt(ratio) * ddz_upt      
      ...
      Kmodel = 0.45d0
      H = (rcen/r0)**exponent
      Tpz_model = -Kmodel * dataM(ir, iz, irhob) * sqrt(ratio) * H * ddz_upt

      ! Correlation coefficient:
      product = dataM(ir,iz,iTpp) * dataM(ir,iz,iTrr)
      C_r_phi = dataM(ir, iz, iTpr) / sqrt(abs(product))
      write(lun_model, "(6(1x, e12.5))") rcen, zcen, Tpz_model, C_r_phi, product, ddz_upt
   end do
end do
