...
! Note: fargo_factor is caclulated in subroutinr rhs which is contained
! rhs.f90
...
! This routine is called by subroutine rk4 (in time_stepping_routines.f90) at the beginning of a time step
! (if Fargo is to be applied).

! Note:  For integer shifts, u_phi_fargo_subtract changes with time but is 
...
if (integer_shifts) then
   ...
   do ir = 1, nr
      desired_phi_shift(ir) = Omega_bar(ir) * dt
      ! This calculation is redundant at every time step (since it is independent
      ! of time.  At some point we should modify the code so it is done only once.
...
subroutine add_fargo_extra_operator_terms(q, qdot, p_phi_space, lambda_extra_operator_max)
...
real(8), dimension(sr:er, sz_phi:ez_phi,       nphi), intent(in)    :: p_phi_space
! Maximum eigenvalue (for this processor).  To be used for time step selection.
...
do iphi = 1, nphi
   do iz = sz_phi, ez_phi
      do ir = sr, er
         ...
         F(ir, iz, rmom, iphi) =  rgrid(ir)*(p_phi_space(ir,iz,iphi) + q(ir, iz, rmom, iphi)*ur)
         ...
         if (p_phi_space(ir, iz, iphi) .lt. 0.d0) then
            print *, ' p_phi_space(ir, iz, iphi) = ', p_phi_space(ir, iz, iphi), ' < 0'
            print *, ' ir = ', ir, ' iz = ', iz, ' iphi = ', iphi
         end if
         c_sound = SQRT(gamma_sound_speed * p_phi_space(ir, iz, iphi) / q(ir, iz, irho, iphi))
...
! Differentiate w.r.t. phi.  The ndof + 1 is to accomodate div_u:
   ...
   do iz = sz_phi, ez_phi
      do ir = sr, er
         qdot(ir, iz, ener, iphi) = qdot(ir, iz, ener, iphi) - &
                                    p_phi_space(ir, iz, iphi) * dF(ir, iz, div_u, iphi) * fargo_factor_over_r(ir)
      end do
   end do
end do
