...
! average up the phi Reynolds stresses.
...
integer, parameter :: nbar = 10

! The ten -- now eleven phi averages we compute:
real(8), dimension(nbar, sr:er, sz_phi:ez_phi) :: bar

! Indices for the ten -- now eleven quantities:
! March 9, 2024: Added uphi bar to test Manger etal expression:
integer, parameter :: rho = 1, rho_uphi= 2, rho_ur = 3, rho_uz = 4, &
     rho_uphi_ur = 5, rho_uphi2 = 6, rho_ur2 = 7, rho_uz2 = 8, &
     rho_uphi_uz = 9, rho_ur_uz = 10, uphi_index = 11

! These are for performing z averages (on top of phi averages) if required.
real(8), dimension(nbar, sr:er) :: bar_z_sum
real(8), dimension(nbar, nr)    :: bar_z_sum_final
real(8), dimension(nbar)        :: buffer
integer :: i

character(11), parameter, dimension(nbar) :: &
     names = ['rho        ', &
              'rho_uphi   ', &
              'rho_ur     ', &
              'rho_uz     ', &
...
do iz = sz_phi, ez_phi
   do ir = sr, er
      do iphi = 1, nphi
         ...
         ! bar(uphi_index,  ir,iz) = bar(uphi_index,  ir,iz) + uphi                
      end do
   end do
end do
bar = bar/nphi
...
open (unit = lun_general_purpose, file = filename, form = 'formatted', status = 'unknown', &
      access = 'direct', recl = 17*nbar + 1) ! nbar quantities + char(10)

do iz = sz_phi, ez_phi
   do ir = sr, er
      irec = ir + (iz-1)*nr  
      write(lun_general_purpose, "(10(1x, e16.9), a1)", rec = irec) &
      (bar(ivar, ir, iz), ivar = 1, nbar), char(10)
   end do
end do
close(lun_general_purpose)
...
if (average_in_z) then
   ...
   ! tag = ir
   do ir = sr, er
      call mpi_send(bar_z_sum(1, ir), nbar, mpi_double, 0, ir, mpi_comm_world, ier)
   end do
   ...
   if (my_node .eq. 0) then
      ...
      do ir = 1, nr      
         do i = 1, ng2
            ...
            call mpi_recv(buffer(1), nbar, mpi_double, mpi_any_source, ir, &
