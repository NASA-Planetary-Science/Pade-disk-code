...
subroutine app_vortex_pair
! Vortex pair in vertically integrated disk.  Can be used to test Fargo.
...
real(8) :: t, cfl, dt_used, dt_supplied, dt_previous
...
logical :: apply_plotting_shift = .false., plot_curl_rho_u = .false.
...
namelist /vp_input/ restart, nr, nphi, nz, cfl, nsteps, hit_target, &
     t_target, tecplot_interval, tau_filter, apply_fargo_trick, integer_shifts, &
     apply_fargo_correction, use_supplied_dt, dt_supplied, isothermal, &
...
if (my_node .eq. 0) then
   ...
   call mpi_bcast(dt_supplied,            1, mpi_double,  0, mpi_comm_world, ier)
...
if (.not. restart) then
   call add_vortex_pair (vortex_type, Gamma_Gaussian, sigma_Gaussian, GM, Gamma_Seligman, &
        k_delta, n_wedges)
   if (my_node .eq. 0) print *, ' app_vortex_pair: Returning from add_vortex_pair'
end if
...
do istep = istep1, istep1 + nsteps - 1
   call rk4(q, cfl, t, dt_used, use_supplied_dt, dt_supplied, unphysical, iphi_bad, &
   ...
   ! Detect blowing-up:
   if (istep .gt. 1) then
      if (dt_used .lt. dt_previous/20.d0) then
...
do i = 1, n_points
   ...
   u_theta_vec(i) = u_theta_func(my_node,vortex_type, pi, Gamma_Gaussian, sigma_Gaussian, &
      ...
      do ir = sr, er
         ...
         do ivort = 1, 2
            do i_wedge = 1, n_wedges
               ...
               u_theta = u_theta_func(my_node,vortex_type, pi, Gamma_Gaussian, sigma_Gaussian, &
...
real(8) function u_theta_func(my_node,vortex_type, pi, Gamma_Gaussian, sigma_Gaussian, &
     Gamma_Seligman, k_delta, delta, s, s_cut_off, s_max_Melander)

implicit none
integer      :: my_node ! For debug output only
character(8) :: vortex_type
...
use partition_data, only: my_node
implicit none
real(kind=8) :: s, pi, Gamma, sigma_Gaussian

! Locals:
real(kind=8) :: arg, exp_arg

arg = -s**2/sigma_Gaussian**2
exp_arg = exp(arg)
gaussian_u_theta = Gamma / (2.d0*pi*s) * (1.d0 - exp_arg)
...
!#ifdef debug_print
!   if (my_node .eq. 0) then
!      print *, ' About to compute bracket and Melander3'
!      print *, '    paren = ', paren
!      print *, '    exp(paren) = ', exp(paren)
!   end if
!#endif   
