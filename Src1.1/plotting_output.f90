...
real(8) function c_sound_at_point(caller, q, ir, iphi, iz)
...
if (isothermal) then
   ...
   if (isnan(c_sound_at_point)) then
      print *, ' In function c_sound_at_point'
      print *, ' Caller = ', caller
...
do ir = 1, nr
   ...
   p  = pressure_at_point(q, ir, iphi, iz)
   ci = SQRT(p/rho)
   cs = c_sound_at_point('radial_profiles               ', q, ir, iphi, iz)
...
subroutine tecplot_cylinder(q, ir, t, istep)
...
if (my_node .eq. 0) then
   ...
   2  format('VARIABLES = "phi", "z", "rho", "ur", "uz", "uphi"', a1)
   write (lun_tecplot, 3, rec = 3) nphi, nz, char(10)
   3 format('ZONE I=', i4, ',', ' J=',i4, ' DATAPACKING=POINT', a1)
end if

! Write only of this processor has the iphi:
i_have_ir = (ir .ge. sr) .and. (ir .le. er)
if (i_have_ir) then
   do iz = 1, nz
      do iphi = sphi, ephi
         rho    = q(ir, iphi, iz, irho)
         ...
         irec = (iz - 1)*nphi + iphi + 3

         write(lun_tecplot, "(6(1x, e12.5), a1)", rec = irec) phi_grid(iphi)/pi, &
              zgrid(iz)/pi, rho, ur, uz, uphi, char(10)
      end do
   end do
end if
...
if (my_node .eq. 0) then
   print *, ' rank 0: In subroutine tecplot_cylinder wrote tecplot file ', &
        filename
end if

end subroutine tecplot_cylinder

!----------------------------------------------------------------------------------85

subroutine tecplot_meridional_plane(q, iphi, L_scale, T_scale, t, istep)

! iphi identifies the meridional plane to plot.
...
if (i_have_iphi) then
   do iz = 1, nz
      do ir = sr, er
         ...
         cs   = c_sound_at_point('tecplot_meridional_plane      ', q, ir, iphi, iz)
         Mach_r = ur/cs
         Mach_z = uz/cs
...
if (sphi .eq. 1) then
   ...
   do ir = sr, er
      ...
      ! Note: I have stored the coordinates for this line in the iphi = 0 slot
      ! since the flow data sits in the same processor:
      write(lun_tecplot, "(9(1x, e12.5), a1)",rec=irec) xgrid(ir,0),ygrid(ir,0),&
           rho, rho_prime, ur, uphi, uphi_prime, uz, ci, char(10)
   end do
end if
...
integer :: ir, iz, irec
real(8) :: rho, rho_prime, ur, uz, uphi, uphi_prime, d_uphi2
! Function called:
real(8) :: t_scaled, U_scale
logical :: i_have_iphi
...
! recl is the maximum record length.  7 variables
open (unit = lun_tecplot, file = filename, form = 'formatted', status = 'unknown', access = 'direct', recl = 13*7 + 1)
...
if (my_node .eq. 0) then
   ...
   1  format ('TITLE = "t = ', e12.5, '"', a1, a1)
   write (lun_tecplot, 2, rec = 2) char(13), char(10)
   2  format('VARIABLES = "r", "z", "drho", "ur", "uz", "uphi", "d(uphi^2)"', a1, a1)
   write (lun_tecplot, 3, rec = 3) nr, nz, char(13), char(10)
   3 format('ZONE I=', i4, ',', ' J=',i4, ' DATAPACKING=POINT', a1, a1)
end if
...
if (i_have_iphi) then
   do iz = 1, nz
      do ir = sr, er
         ...
         d_uphi2 = uphi**2 - uphi_basic(ir, iz)**2
      
         irec = (iz - 1)*nr + ir + 3

         write(lun_tecplot, "(7(1x, e12.5), a1)", rec = irec) rgrid(ir)/L_scale, &
              zgrid(iz)/L_scale, rho_prime, ur/U_scale, uz/U_scale, uphi_prime/U_scale, &
              d_uphi2/U_scale**2, char(10)
      end do
   end do
end if
close(lun_tecplot)
