...
use basic_state, only: eint_basic ! For relaxation via Newton's law of cooling
...
! Eigenvalue for the extra operator.
...
real(8) :: eint_target ! For Newtonian cooling, which is just a relaxation to a target.
...
! Code for advecting the density by a fixed velocity:
...
lambda_max_euler_fargo           = 0.d0 ! Max eigenvalue for the Euler terms with fargo
lambda_max_euler_non_fargo       = 0.d0 ! Same as above except without fargo
...
! This is the RHS matrix, which will be accumulated, term by term:
qdot = 0.0d0
...
! Add in z-derivatives in the inviscid equations:
...
#endif
   ! This subroutine is located in add_derivative_routines.f90
...
! Add in gravity terms in the momentum equations:   
...
! Add in centrifugal term in radial momentum eq.:
do iz = 1, nz
   do iphi = sphi, ephi
      do ir = sr, er
...
#endif
   ! This subroutine is located in add_derivative_routines.f90.
   ! Note: phi derivatives will include the extra operator for Fargo. 
      ...
      do iphi = sphi, ephi
         do ir = sr, er
            qdot(ir,iphi,iz,ener) = qdot(ir,iphi,iz,ener) + &
                     (eint_basic(ir,iz) - q(ir,iphi,iz,ener)) / tau_newtonian_cooling
         end do
      end do
   end do
   ! Include tau_newtonian_cooling for time step restriction:
