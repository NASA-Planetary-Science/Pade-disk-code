...
! subroutine set_up_boundary_conditions is located in set_up_routines.f90
...
module boundary_condition_data
   ...
   ! This is for balanced non-reflective isothermal BC.  Dimension will be nz.
   ! Allocation is done in set_up_boundary conditions located in set_up_routines.f90
   real(8), allocatable, dimension(:) :: d_ci_dr_inner, d_ci_dr_outer

   ! These are for dirichlet BC at zmin and zmax (BC type = z_dirichlet):
   real(8), allocatable :: qLeft_z_BC(:), qRight_z_BC(:)
   
end module boundary_condition_data
...
if (sr .eq. 1) then ! This checks if this proc has the boundary.
   ...
   if (rmin_BC .eq. zero_normal_momentum) then
      ...
      do iz = 1, nz
         do iphi = sphi, ephi
            q(ir, iphi, iz, rmom) = 0.0d0
            q(ir, iphi, iz, zmom) = q(ir,iphi,iz,irho) * uz_rmin
...
! r max BC:
!~~~~~~~~~~
if (er .eq. nr) then ! If my cpu has r = nr
   ir  = nr
   if (rmax_BC .eq. zero_normal_momentum) then
      ...
      do iz = 1, nz
         do iphi = sphi, ephi
            q(ir, iphi, iz, rmom) = 0.0d0
            q(ir, iphi, iz, zmom) = q(ir,iphi,iz,irho) * uz_rmax
...
do iphi = sphi, ephi
   do ir = sr, er
      cs   = c_sound_at_point('z_adiabatic_non_reflective_bc ', q, ir, iphi, iz)
      uz1  = uz(ir, iphi, iz)
