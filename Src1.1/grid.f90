...
else if ((nr .eq. 1) .and. (nphi .ne. 1) .and. (nz .ne. 1)) then
   ! phi and z only case.  Assumes no variation in phi grid size:
   do iz = 1, nz
      min_grid_size(1, iz) = min(dz(iz), r_dphi(1))
      l_grid_squared(ir, iz) = dz(iz) * r_dphi(1)      
   end do
else
