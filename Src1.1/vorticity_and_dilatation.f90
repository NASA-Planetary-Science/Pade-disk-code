...
subroutine tecplot_vort_z_Manger_style(q, iz_plot, Omega_K, t, istep)

! Outputs vort_z / Omega_K

! iz_plot = desired horizontal plane.
...
real(8) :: vort_z_rel_norm

#ifdef debug_print
if (my_node .eq. 0) then
   print *, ' node 0: tecplot_vort_z_rel_norm_in_merid_horiz_planes(has been called'
end if
#endif
...
if ((iz_plot .ne. 0) .and. (nphi .ne. 1)) then
   write (filename, 81) iz_plot, int(t), t - int(t), istep
81 format ('vort_z_Manger_style_iz_', i4.4, '_t_', i6.6, f0.4, '_', i7.7, '.tec')
   
   open (unit = lun_tecplot, file = filename, form = 'formatted', status = 'unknown', &
        access = 'direct',    recl = 13*3 + 1)
   ...
   if (my_node .eq. 0) then
      ...
      21  format('VARIABLES = "r", "phi", "wz/OmK"', 2a1)
   ...
   do iphi = sphi, ephi
      do ir = sr, er
         irec = (iphi - 1)*nr + ir + 3
         vort_z_rel_norm = v(ir, iphi, iz_plot, z_comp) / Omega_K(ir)
         ! Why no char(13)
         write(lun_tecplot, "(3(1x, e12.5), a1)", rec = irec) &
                 rgrid(ir), phi_grid(iphi), vort_z_rel_norm, char(10)      
      end do
   end do
   ...
   if (sphi .eq. 1) then
      ...
      do ir = sr, er
         ...
         vort_z_rel_norm = v(ir, iphi, iz_plot, z_comp) / Omega_K(ir)
         write(lun_tecplot, "(3(1x, e12.5), a1)", rec = irec) &
                 rgrid(ir), 2.d0*pi, vort_z_rel_norm, char(10)      
      end do         
   end if
   
   close(lun_tecplot)
   if (my_node .eq. 0) then
      print *, ' node 0: In subroutine tecplot_vort_z_Manger_style'
      print *, ' wrote tecplot file =', filename
   end if
end if

end subroutine tecplot_vort_z_Manger_style

!----------------------------------------------------------------------------------85

subroutine tecplot_vort_z_rel_norm_in_merid_horiz_planes(q, iphi_plot, iz_plot, &
                                                         Omega_K, t, istep)

! Definition: relative_vort_z = (vort_z - 2 Omega_K) / (2 Omega_K)
...
if (iphi_plot .ne. 0) then
   ...
   write (filename, 74) iphi_plot, int(t), t - int(t), istep
74 format ('rel_vort_z_norm_iphi_', i4.4, '_t_', i6.6, f0.4, '_', i7.7, '.tec')         
   
   open (unit = lun_tecplot, file = filename, form = 'formatted', &
         status = 'unknown', access = 'direct',recl = 13*3 + 1)
   ...
   if (my_node .eq. 0) then
      ...
      2  format('VARIABLES = "r", "z", "wz_rel_norm"', 2a1)
   ...
   if (i_have_iphi_plot) then
      do iz = 1, nz_plot
         do ir = sr, er
            irec = (iz - 1)*nr + ir + 3
            vort_z_rel_norm = (v(ir, iphi_plot, iz, z_comp) - 2.d0*Omega_K(ir)) / (2.d0*Omega_K(ir))
            ! Why no char(13) = CR
            write(lun_tecplot, "(3(1x, e12.5), a1)", rec = irec) &
                 rgrid(ir), zgrid(iz), vort_z_rel_norm, char(10)      
         end do
      end do
   end if
   close(lun_tecplot)
   if (my_node .eq. 0) then
      print *, ' node 0: In subroutine tecplot_vort_z_rel_norm_in_merid_horiz_planes'
      print *, '         wrote tecplot file =', filename
   end if
end if
...
if ((iz_plot .ne. 0) .and. (nphi .ne. 1)) then
   write (filename, 81) iz_plot, int(t), t - int(t), istep
81 format ('vort_z_rel_norm_iz_', i4.4, '_t_', i6.6, f0.4, '_', i7.7, '.tec')
   ...
   if (my_node .eq. 0) then
      ...
      11  format ('TITLE = "t = ', e12.5, '"', 2a1)
      write (lun_tecplot, 21, rec = 2) char(13), char(10)
      21  format('VARIABLES = "x", "y", "wz_rel_norm"', 2a1)
      write (lun_tecplot, 31, rec = 3) nr, nphi+1, char(13), char(10)
      31 format('ZONE I=', i4, ',', ' J=',i4, ' DATAPACKING=POINT', 2a1)
   end if
   ...
   do iphi = sphi, ephi
      do ir = sr, er
         irec = (iphi - 1)*nr + ir + 3
         vort_z_rel_norm = (v(ir, iphi, iz_plot, z_comp) - 2.d0*Omega_K(ir)) / (2.d0*Omega_K(ir))
         ! Why no char(13)
         write(lun_tecplot, "(3(1x, e12.5), a1)", rec = irec) &
                 xgrid(ir,iphi), ygrid(ir,iphi), vort_z_rel_norm, char(10)      
      end do
   end do
   ...
   if (sphi .eq. 1) then
      ...
      do ir = sr, er
         ...
         ! since the flow data sits in the same processor.  Why didn't you use
         ! e.g., xgrid(ir,1)?
         ! Why no char(13)?
         write(lun_tecplot, "(3(1x, e12.5), a1)", rec = irec) &
                 xgrid(ir,0), ygrid(ir,0), vort_z_rel_norm, char(10)      
      end do         
   end if
...
end subroutine tecplot_vort_z_rel_norm_in_merid_horiz_planes
...
74       format ('total_vort_and_dil_iphi_', i4.4, '_t_', i6.6, f0.4, '_', i7.7, '.tec')         
   ...
   close(lun_tecplot)
   if (my_node .eq. 0) then
      print *, ' node 0: In subroutine tecplot_vort_and_dil_in_merid_horiz_planes'
      print *, '         For meridional plane plot wrote tecplot file =', filename
   end if 
end if
...
do iphi_plane = 1, nphi_planes
   ...
   close(lun_tecplot)
   if (my_node .eq. 0) then
      print *, ' node 0: In subroutine tecplot_vort_and_dil_in_many_merid_horiz_planes'
      print *, ' iphi = ', iphi, ' iphi_plane = ', iphi_plane, ' nphi_planes = ', nphi_planes
      print *, ' wrote tecplot file =', filename
   end if
end do
...
do iz_plane = 1, nz_planes
   ...
   if (my_node .eq. 0) then
      print *, ' node 0: In subroutine tecplot_vort_and_dil_in_many_merid_horiz_planes'
      print *, '         For iz = ', iz, ' wrote tecplot file =', filename
   end if
end do
...
if (.not. compute_curl_rho_u) then
   ...
   7 format (a6, '_vort_and_dil_t_', i6.6, f0.6, '.tec')   
else
   write (filename, 71) prefix, int(t), t - int(t)
   71 format (a6, '_curl_rho_u_and_dil_t_', i6.6, f0.6, '.tec')   
end if
...
5     format(' VARIABLES = "x", "y", "z", "wz", "dil"', a1, a1)            
   ...
   ! Note: nphi + 1 for periodic completion:
